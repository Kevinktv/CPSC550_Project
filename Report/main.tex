...
\end{itemize}
High FPR and FNR indicate that attacks cannot reliably distinguish between the two models, implying strong indistinguishability. We convert these rates into one final score $\epsilon^s$ where smaller $\epsilon^s$ indicates better indistinguishability. We use the strongest (worst-case) attack for each example.
...
We measure utility of a model, $O$ as the accuracy on the retain set ($D \setminus S$) and a held-out test set ($T$). 
\begin{align}
