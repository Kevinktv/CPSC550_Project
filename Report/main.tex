...
This project aims to benchmark several recent approximate machine unlearning algorithms against a retraining baseline. We focus on image classification tasks, and will be focusing on the following unlearning algorithms: 
...
\textbf{Kaggle 2023 Winner (Fanchuan)} [\cite{2023-winner}]: Fanchuan is a two-stage unlearning algorithm. In the first stage, the algorithm minimizes the KL divergence of the models output on the forget set and a uniform distribution. This encourages the model to produce less confident predictions on the forget set. In the second stage, the algorithm performs adversial fine tuning using contrastive learning to push the forget samples away from the retain samples in the feature space. They simultaneously also reduce the cross entropy loss on the retain set to preserve performance on the retain set. 
...
For the unlearning task, the training partition is divided into two mutually exclusive sets:
\begin{enumerate}
...
As mentioned earlier, it is intractable to compare full model distributions, so indistinguishability is estimated per example $s \in S$. We apply various ``attacks'' on the model outputs to distinguish outputs of unlearned models from retrained models, yielding a False Positive Rate (FPR) and False Negative Rate (FNR). For each attack:
\begin{align}
    \epsilon_1 &= \log(1 - \delta - \text{FPR}) - \log(\text{FNR}) \\
    \epsilon_2 &= \log(1 - \delta - \text{FNR}) - \log(\text{FPR})
\end{align}
The final score - $\epsilon^s$ for example $s$ is derived from the maximum value (representing the worst-case, strongest attack):
\begin{equation}
    \epsilon^s = \max(\epsilon_1, \epsilon_2)
\end{equation}
Smaller $\epsilon^s$ indicates better forgetting quality.
...
\subsection{Benchmark result tables}

\begin{table*}[htbp]
    \centering
    \small
    \caption{Appendix benchmark summary for CIFAR-10. Final Score is shown as N/A when the runtime efficiency threshold is not passed.}
    \label{tab:appendixBenchmarkCifar10}
    \resizebox{\textwidth}{!}{%
...
\begin{table*}[htbp]
    \centering
    \small
    \caption{Appendix benchmark summary for MUFAC. Final Score is shown as N/A when the runtime efficiency threshold is not passed.}
    \label{tab:appendixBenchmarkMufac}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccccccc}
        \toprule
        Method & Forgetting Quality & Retain Accuracy & Test Accuracy & Retain Ratio & Test Ratio & Pass Threshold & Final Score \\
