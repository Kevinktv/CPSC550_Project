...
This project compares their qualitative performance against the 2023 winning solution (\cite{2023-winner}) aims to provide a survey of the different approaches.
...
We will mainly focus on unlearning in the context of image classification on CIFAR-10 [\cite{Krizhevsky09learningmultiple}], Kaggle NeurIPS 2023 Machine Unlearning dataset [\cite{neurips-2023-machine-unlearning}], and MUFAC (Machine Unlearning Facial Attribute Classification) dataset [\cite{choi2023machine}]. 
\section{What do you plan to do?}
We plan to do a comparative review and benchmark of more recent unlearning classifier task. We will use ResNet-18 as a baseline model, retrain from scratch to get an exact unlearning baseline, and evaluate the same metrics used in Metric used for Kaggle competition (\cite{neurips-2023-machine-unlearning}).

The algorithm we plan to compare are:
\begin{itemize}
    \item The 2023 NeurIPS Unlearning Competition winning solution [\cite{2023-winner}]
    \item Masked Small Gradient + Convolution Transpose (CT) [\cite{cadet2025}]
...
The main contribution of this project will be a clear and reproducible benchmark of recent machine unlearning algorithms on classification tasks under the same evaluation framework.
\printbibliography
\end{document}
